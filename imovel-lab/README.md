...
Auto (default) adapts the splat budget during movement: initially 2.5 million on desktop / 1.5 million on mobile, decreasing if measured movement falls below 32 fps and increasing above 52 fps. When stationary, it refines to 7 million / 3 million respectively. High locks 8 million; Ultra locks 12 million. High and Ultra use the larger 32-byte/splat intermediate format to reduce additional rendering quantization; Auto retains the 20-byte compact format for bandwidth. Ultra also retains splats down to 0.25 pixels with a 0.5 contribution threshold. These are budgets, not guaranteed frame rates. Resolution stays at native CSS pixels or higher (device pixel ratio capped at 2); there is no sub-native resolution scaling. New preferences use rdc-detail-v2 so older fixed presets do not silently disable Auto.
